...
class SRNet():
    ...
    def __init__(self, shape = [512, 512], name = ''):
        #print(shape)

        self.name = name
        self.cnum = 32
        self.ssim = []
        self.ssim_multiscale = []
        self.graph = tf.Graph()
    ...
    def build_generator(self, inputs, name = 'generator'):
        ...
        """shape = i_s.shape
        im1 = np.resize(np.asarray(i_s.numpy()), shape)
        im2 = np.resize(np.asarray(o_f.numpy()), shape)
        ...
        a = tf.image.ssim(im1, im2, max_val=255, filter_size=4,
                              filter_sigma=1.5, k1=0.01, k2=0.03).numpy()

        ssim += a[0]
        
        b = tf.image.ssim_multiscale(img1=im2, img2=im1, max_val=255, filter_size=4).numpy()
        ssim_multiscale += b[0]
        """

        return o_sk, o_t, o_b, o_f,0,0 #ssim, ssim_multiscale 
    ...
    def build_whole_net_with_loss(self):
        ...
        o_sk, o_t, o_b, o_f,  ssim, ssim_multiscale = self.build_generator(inputs)
        
        self.ssim.append(ssim)
        self.ssim_multiscale.append(ssim_multiscale)
        ...
        with open(vgg_graph_path, 'rb') as f:
            ...
            # print(vgg_graph_def)
            all_names = [op.name for op in vgg_graph_def.node]
            #print(all_names)
            with open(vgg_graph_path, 'rb') as f:
                vgg_graph_def.ParseFromString(f.read())
        ...
        self.g_loss_detail = self.g_loss_detail 
     
    def build_optimizer(self):

        #self.learning_rate = tf.train.cosine_decay_restarts(cfg.learning_rate, self.global_step, first_decay_steps=cfg.decay_steps, alpha=1e-2)
        ...
        with tf.control_dependencies(d_update_ops):
            self.d_train_step = tf.train.GradientDescentOptimizer(self.learning_rate).minimize(self.d_loss,
                var_list = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = 'D'))
        g_update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='G')
        with tf.control_dependencies(g_update_ops):
            self.g_train_step = tf.train.GradientDescentOptimizer(self.learning_rate).minimize(self.g_loss,
                var_list = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = 'G'))
        """
    ...
    def train_step(self, sess, global_step, i_t, i_s, t_sk, t_t, t_b, t_f, mask_t):
        ...
        with self.graph.as_default():
            ...
            #print('as_default')
            _, g_loss, g_log, g_loss_detail = sess.run([self.g_train_step, self.g_loss, self.g_summary_op, self.g_loss_detail], feed_dict = feed_dict)
        return d_loss, g_loss, d_log, g_log, g_loss_detail
